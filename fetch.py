...
Tests RSS parsing and data formatting, then sends new routines to reciever mail
"""
...
def is_new_routine(routine_data):
    ...
    print(f"  → Current time: {current_norwegian_time.strftime('%Y-%m-%d %H:%M:%S')}")
    logging.info(f"  → Hours since published: {hours_since_published:.2f}")
    print(f"  → Hours since published: {hours_since_published:.2f}")
    ...
    if hours_since_published <= 7 * 24:
        ...
        print(f"  → This routine is NEW! (published {hours_since_published:.2f} hours ago)")
        ...
        print(f"  → This routine is too old (published {hours_since_published:.2f} hours ago)")
        return False
...
def callMailFunction(routines):
    ...
    try:
        ...
        print(f"📧 Sending {len(routines)} new routines to support mail...")
    ...
    try:
        ...
        for i, entry in enumerate(feed.entries, 1):
            ...
            if is_new_routine(routine_data):
                ...
                print(f"  ⏸️  This routine is already processed, stopping process...")
                break
        ...
        if new_routines:
            ...
            logging.info(f"\n📤 Sending all new routines in one mail")
            print(f"\n📤 Sending all new routines in one mail")
